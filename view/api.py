...
def generate():
    ...
    if auxiliary.empty(signature):
        ...
        if signature.lower() == 'api':
            ...
            return core.generateResponseResult('特征码不能为index')
        elif signature.lower() == 'query':
            return core.generateResponseResult('特征码不能为query')
        elif signature.lower() == 'doc':
            return core.generateResponseResult('特征码不能为doc')
        elif not signature.isdigit() and not signature.isalpha() and not signature.isalnum():
            return core.generateResponseResult('特征码仅能填数字和字母')
        elif len(signature) < 1 or len(signature) > 5:
            return core.generateResponseResult('特征码长度只能为1-5')
